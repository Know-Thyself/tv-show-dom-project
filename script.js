...
function populatePage(arr) {
	arr.forEach(elem => {
		...
		createImage(elem, figure, elem.id)
		if (elem.genres) {
			createShowInfo(
				elem.genres,
				elem.status,
		...
		wrapper.addEventListener('click', containerEvent)
	})
}
...
const createImage = (obj, container, id) => {
...
const containerEvent = e => {
	...
	const currentShow = sortedShows.filter(show => show.id === parseInt(showId))
	currentShowName = currentShow[0].name
...
const episodeNameEvent = e => {
	let clickedEpisode = episodes.filter(episode => {
		return episode.name === e.target.innerText.split(' ').slice(0, -2).join(' ')
	})
	resetEpisodeSelect()
